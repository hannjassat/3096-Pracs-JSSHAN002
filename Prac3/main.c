...
// indication of button toggle: default (false) correpsonds to 1Hz and true corresponds to 2Hz
bool blueButton= false;
// value is set based on blue button toggle 
float delayValue;
/* USER CODE END PV */
...
int main(void)
{
  ...
  while (1)
  {
      ...
      unint32_t adcVal = pollADC(); // returns the most recent ADV value
      sprintf(buffer, "\r\n %d", adcVal); 
      HAL_UART_Transmit(&huart2, buffer, sizeof(buffer), 1000);

	  //TASK 3
	  //Test your ADCtoCRR function. Display CRR value via UART
      unint32_t crrVal = ADCtoCRR(adcVal);  // returns the converted ADC value to PWM duty cycle value
      sprintf(buffer, "\r\n %d", crrVal);
      HAL_UART_Transmit(&huart2, buffer, sizeof(buffer), 1000);
...
void pause_sec(float x)
{
    ...
    // causes a delay while iterating through nested loop
    volatile int i, j;
    for (i=0; i<=x*DELAY1;i++) // iterates 500 times
        for (j=0; j<=DELAY2;j++); // iterates 6000 times 
...
void EXTI0_1_IRQHandler(void)
{
    ...
    if(blueButton) // set as alternate to default setting: 2Hz
    {
        blueButton = false;  // ensures that the next iteration will set delay value to 1Hz
        delayValue = 0.5;
    }else{ // default setting: 1Hz 
        blueButton = true;  // ensures that the next iteration will set delay value to 2Hz
        delayValue = 1;
    }

    // introduces a fixed timeout
    timeout = HAL_GetTick() + pause_sec(delayValue) ;
    while(ProcessOngoing)
    {
        // compares the provided tick value with the fixed timeout
        if(HAL_GetTick() >= timeout) 
        {
          // when the tick is larger than the fixed timeout the pin is toggled 
...
uint32_t pollADC(void){
	//TO DO:
	//TASK 2
    //start, poll and stop the ADC and return the most recent ADC value
...
uint32_t ADCtoCRR(uint32_t adc_val){
    //converts an ADC value to a PWM duty cycle value
	//TO DO:
	//TASK 2
	// Complete the function body
  // ratio of adc val is used to get the crr relative to max arr (47999)
    uint32_t ccr = 47999 * ((adc_val/4096));
    ...
    uint32_t val = __HAL_TIM_SetCompare(&htim3, TIM_CHANNEL_4, ccr);
	return val;
}
